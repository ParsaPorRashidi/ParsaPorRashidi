# Hello There , I'm Parsa Pourrashidi 😃

----

## 🧠 ABOUT ME
   🏡 Based in Tabriz | Backend Developer 

<p align="center">
  <img align="right" src="https://github.com/7oSkaaa/7oSkaaa/blob/main/Images/Right_Side.gif?raw=true" width="250px">
</p>

- 💻 Backend Developer focused on building scalable and clean server-side systems.
- 🌍 Passionate about creating and maintaining open-source projects to share knowledge and grow with the community.
- ⚙️ Currently diving deeper into task systems, background jobs, and asynchronous architectures.
- 🚀 Always learning, refining, and pushing myself to become a stronger engineer every day.
- 🧠 2026 mission: Build impactful open-source tools and master production-grade backend architecture.

<br clear="right"/>

---

## 🛠️ TECH STACK & TOOLS

<div align="center">

[![Skills](https://skills.syvixor.com/api/icons?i=python,django,drf,postman,mysql,git,github)](https://github.com/syvixor/skills-icons)
[![Skills2](https://skills.syvixor.com/api/icons?i=xubuntu,celery,discord,html,css,docker,pycharm)](https://github.com/syvixor/skills-icons)

</div>


## 🤖 Learning
<div align="center">

[![Skills3](https://skills.syvixor.com/api/icons?i=rabbitmq,n8n,postgresql)](https://github.com/syvixor/skills-icons)

</div>

---

## 🚀 FEATURED PROJECTS

### 🔹 Telegram Toolze (Open Source)
Production-ready task processing system built with Django, Celery & RabbitMQ.

### 🔹 Discord Server Manager (Open Source)
Advanced asynchronous bot system using discord.py & background workers.

---

## 🌐 CONNECT WITH ME

<div align="center">

<a href="YOUR_LINKEDIN" target="_blank">
  <img src="https://skills.syvixor.com/api/icons?i=linkedin" />
</a>
<a href="YOUR_LINKEDIN" target="_blank">
  <img src="https://skills.syvixor.com/api/icons?i=telegram" />
</a>
<a href="YOUR_LINKEDIN" target="_blank">
  <img src="https://skills.syvixor.com/api/icons?i=instagram" />
</a>

</div>

## 🌐 Website

<div align="center">

<a href="https://back-blaze.ir" target="_blank">
  <img src="image/backblaze.png" height="48" style="border-radius:16px;"/>
</a>

</div>

---